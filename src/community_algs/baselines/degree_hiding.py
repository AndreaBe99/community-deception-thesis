...
class DegreeHiding():
    ...
    def __init__(
        ...
        self.detetion_alg = CommunityDetectionAlgorithm(detetion_alg)
        self.original_community = self.detetion_alg.compute_community(graph)
    ...
    def hide_target_node_from_community(self) -> tuple:
        ...
        while self.steps > 0 and not done:
            ...
            max_tuple = max(self.possible_edges, key=lambda x: x[2])
            self.possible_edges.remove(max_tuple)
            ...
            communities = self.detetion_alg.compute_community(graph)
            idx = self.get_community_target_idx(
                communities.communities, self.target_community)
        ...
        for v in self.graph.nodes():
            ...
            if in_community(u) and in_community(v):
                if self.graph.has_edge(u, v):
                    possible_edges.add((u, v, self.graph.degree(v)))
