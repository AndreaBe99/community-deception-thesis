from src.community_algs.metrics.modularity import NODEC
...
class ExperimentsNODEC:
    def __init__(
        ...
        self.graph = ig.Graph.from_networkx(graph)
        
        self.nodec = NODEC(graph=self.graph)

        self.current_algo = None
        # self.worst_case_scenario = None
    ...
    def com_decept_nodec(self, budget):
        ...
        while (True):
            ...
            if (beta == 0) or self.nodec.target_community.count(-1) == self.nodec.initial_community_size-2:
                break
            # else:
            #    break
        # Convert graph from igraph to networkx
        return self.nodec.graph.to_networkx()
        # return self.nodec.graph

    def performNodeDeletion(self, node_to_delete):
        neighs = self.nodec.graph.neighborhood(
    ...
    def performNodeMove(self, move_details):
        ...
        degree_in_com_copy[nodex_index_in_com,
    ...
    def runExperiment(self, network, detection_algo, budget, internalnalP, externalP, worst_case_scenario):
        ...
        deception_before = self.nodec.getDeceptionScore(
            after_updates=False)  # after_updates=False
        print("Initial ComH=", self.nodec.target_community)
        print("Initial modularity=", self.nodec.getModularity(
            self.nodec.graph, self.nodec.communities_object))
        print("Initial decScore=", deception_before)
        print("Initial number of nodes and edges=",
              self.nodec.graph.ecount(), self.nodec.graph.vcount())
        ...
        coms_after = self.nodec.computeCommunitiesAfterUpdateCDLIB(
            detection_algo, g)
        num_coms_after = len(coms_after)
        deception_after = self.nodec.getDeceptionScore(
            after_updates=True)  # after_updates=True

        print("Final ComH=", self.nodec.target_community)
        print("Final modularity=", self.nodec.getModularity(
            g, self.nodec.communities_after_object))
        print("Final number of nodes and edges=",
              new_graph.ecount(), new_graph.vcount())
                ...
                for com_algo in self.detection_algos:
                    for internalP, externalP in zip(self.sampleInternalPercentage, self.sampleExternalAddPercentage):
                        for i in range(0, self.rounds):
                            self.runExperiment(
                                network, com_algo, budget, internalP, externalP, self.worst_case_scenario)
        self.file_experiments.close()


# def main():
#     dataset_path = "./datasets/edgelist/"
#     # "terrorist2","terrorist1","blogcatalog" #"pubmed","webkb", "cora", "citeseer","facebook","netscience"
#     datasets = ["terrorist1"]
#     # louv # "walk" "infomap" "labelp" "greedy" # not in the paper "btw","eig","opt"
...
#     num_rounds = 1
#     experiments = ExperimentsNODEC(dataset_path, datasets, detection_algorithms, budget_updates, internal_edge_sample_sizes,
